...
c Wij check for the DM/SM processes
      do x1=1,ndmparticles
        do x2=1,ndmparticles
          do x3=1,scattering_nprocesses(x1,x2)
            
c open up the output file
            call get_process_index(x1,x2,x3,3,proc_index) 
            pname = process_names(proc_index)
            pname_index = index(pname,' ')-1
            output_file = 'output/wij_check_'//pname(1:pname_index)//'.txt'
            open(unit=42, file=output_file)
...
c Wij check output
            ...
            do x4=1, nWij
              write(42,fmt='(2(ES16.8))') betas(x4), Wij_scattering(x1,x2,x3,x4)
            enddo
...
c taacs check for the DM/SM scattering processes
        do x1=1,nvar
          do x2=1,nvar
            do x3=1,scattering_nprocesses(x1,x2)
...
c get the process information to get the initial states and call the taacs_dm2dm integrator
                write(42,fmt='(2(ES16.8))') x, taacs_coupled(x,x1,x2,x3,3)
              enddo
...
c loop over the DM/SM scattering processes
        do x2=1,ndmparticles
          do x3=1,scattering_nprocesses(x1,x2)
            call cross_check_scan(x1,x2,x3,3,x_min,x_max,nsteps,p_or_E,logscale)
          enddo
        enddo
      enddo
...
c get the final state masses using the pmass(i)_(j)_(k).inc files
        if (gorup.eq.1) then
          ...
          call getfsmasses_scattering(pmass,i,j,k)
        endif
...
c loop over all the annihilation diagrams for each DM particle pair
          do x3=1,ann_nprocesses(x1,x2)
            ...
            pname = process_names(process_index)
            pname_index = index(pname,' ') - 1
            cross_section = get_cross_section(roots,x1,x2,x3)
...
c loop over all the DM -> DM diagrams for each DM particle pair
      ...
      write(*,fmt='(A23,ES16.8)') '# Total cross section: ',total_cross
...
c-------------------------------------------------------------------------c
      function cross_check_process(i, j, k, x1init, x2init, p_or_E)
c-------------------------------------------------------------------------c
c                                                                         c
c  This subroutine calculates the annihilation cross section for each     c
...
c parameters used in this subroutine only
      ...
      cross_check_process = get_cross_section(roots,i,j,k)
...
c loop over all the combinations of initial state particles
      do x1=1,ndmparticles
        do x2=x1,ndmparticles
c loop over all the parocesses 
          do x3=1,ann_nprocesses(x1,x2)
            call matrix_element_check_process(x1, x2, x3, x1init, x2init, p_or_E)
          enddo
        enddo
      enddo
...
c-------------------------------------------------------------------------c
      subroutine matrix_element_check_process(i, j, k, x1init, x2init, p_or_E)
c-------------------------------------------------------------------------c
c                                                                         c
...
c opening the appropriate output file
      call get_process_index(i,j,k,process_index)
      pname = process_names(process_index)
      pname_index = index(pname,' ') - 1
