...
c finish the calculation of omega h^2
      ...
      if (print_out) write(*,*) 'Relic density from the canonical density evolution equation ',relic_canon
...
c parameters used in this function only
      double precision T, s, Wij_value, Wij_sum, K2_sum, beta_prime
...
c shift the beta to be appropriate for the combination of dmi and dmj
            beta_prime = dsqrt(1.d0 - ((mdm(x1)+mdm(x2))**2)/s)
            Wij_value = get_Wij_value(beta_prime,x1,x2,0,1)
